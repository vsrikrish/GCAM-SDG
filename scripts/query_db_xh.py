...
classpath = f'{jarlib}:/storage/work/vxs914/GCAM/ensemble_workflow/jar/ModelInterface.jar'
  
for lev in split:
    scenName = '-'.join(l for l in lev.astype(str))
  
    query_out = os.path.join('/gpfs/group/wvp5117/default/GCAM_xh/query_out', f'query_08232021_{scenName}.xls')
    if os.path.isfile(query_out):
        pass
    else:
        write_batch_query(lev, sandbox, '/gpfs/group/wvp5117/default/GCAM_xh/query_out')
        javacmd = f'java -cp {classpath} ModelInterface/InterfaceMain -b {sandbox}/query_08232021_{scenName}.xml'
        os.system(javacmd)
